...
def test_edit_nondestructive(stub, tasks: Mapping[int, str]) -> None:
    task = stub.nondestructive_editTask(task_pb2.Task(id = 11, description = "hi"))
    print(task.description)


def test_edit_destructive(stub, tasks: Mapping[int, str]) -> None:
    task = stub.destructive_editTask(task_pb2.Task(id = 12, description = "how are you"))
    print(task.description)
...
if __name__ == "__main__":
    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        tasks = test_add(stub, 20)
        test_edit_nondestructive(stub,tasks)
        test_edit_destructive(stub,tasks)
